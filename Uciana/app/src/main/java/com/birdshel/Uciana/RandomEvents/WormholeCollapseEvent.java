...
public class WormholeCollapseEvent implements RandomEvent {
    ...
    private boolean check(Game game) {
        ...
        return !hasWormholeCollapseAlreadyHappened(game);
    }
    ...
    private boolean hasWormholeCollapseAlreadyHappened(Game game) {
        for (RandomEventData randomEventData : game.randomEvents.getHistory()) {
            if (randomEventData.getType() == RandomEventType.WORMHOLE_COLLAPSE) {
                return true;
            }
        }
        return false;
    ...
    public boolean checkEvent(Game game) {
        return game.galaxy.getWormholeSystemEndpoints(game.gameSettings.currentRandomEventData1()).contains(Integer.valueOf(game.gameSettings.currentRandomEventData2()));
    }
    ...
    public int execute(Game game) {
        ...
        game.galaxy.removeWormhole(new Point(currentRandomEventData1, currentRandomEventData2));
        game.randomEvents.a(new RandomEventData(RandomEventType.WORMHOLE_COLLAPSE, currentRandomEventData1, currentRandomEventData2, -1));
    ...
    public Entity getMessage(MessageOverlay messageOverlay, int i, int i2, int i3, int i4) {
        ...
        Text text = new Text(0.0f, 0.0f, game.fonts.infoFont, game.getActivity().getString(R.string.wormhole_collapse), new TextOptions(HorizontalAlign.CENTER), buffer);
        ...
        if (messageType == 0) {
            ...
            string = game.getActivity().getString(Functions.getResId("wormhole_collapse_both_" + game.getCurrentPlayer(), R.string.class), new Object[]{name2, name3});
            ...
            string = game.getActivity().getString(Functions.getResId("wormhole_collapse_one_" + game.getCurrentPlayer(), R.string.class), new Object[]{name});
        } else if (messageType != 2) {
            string = "";
        } else {
            string = game.getActivity().getString(Functions.getResId("wormhole_collapse_none_" + game.getCurrentPlayer(), R.string.class));
        }
    ...
    public boolean initialize(Game game) {
        if (check(game)) {
            List<Point> wormholes = game.galaxy.getWormholes();
            if (wormholes.isEmpty()) {
                return false;
            }
            Point point = wormholes.get(Functions.random.nextInt(wormholes.size()));
            game.gameSettings.setSetting(GameSettingsEnum.RANDOM_EVENTS_NEXT_DATA1, Integer.valueOf((int) point.getX()));
            game.gameSettings.setSetting(GameSettingsEnum.RANDOM_EVENTS_NEXT_DATA2, Integer.valueOf((int) point.getY()));
